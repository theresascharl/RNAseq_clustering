...
## 11 cluster according to ICL

##########################################################
# three-way clustering for G-VVI-VV with 11 components ----

# errors occur in single starts and iterations but it still works
set.seed(123)
init <- MatTrans.init(X1, K = 11, n.start = 10)

M <- MatTrans.EM(X1, initial = init, model = "G-VVI-VV", 
                 row.skew = TRUE, col.skew = TRUE, 
                 trans = "None", silent = TRUE, size.control = 10)

save(M, file = "fission_ALR_G-VVI-VV_11.RData")
...
mat1 <- cluster_map(M = M, genes = genes, type = "3way", 
            mprofiles = mprofiles, npcol = 4) 
data1 <- fission_alr_de_flat[rownames(mat1),]
data2 <- fission_mean_profiles_de_flat[rownames(mat1),]

pdf("Figure5.pdf", width = 10, height=6)
par(mfrow=c(2,4), mar=c(1,4.1,2.1,0.1))
matplot(1:10,t(data1[mat1$cluster==1,]),main="cluster 1", ty="l",
        ylab="alr", col=1, lty=1, xlab="",ylim=c(-3,5.5), xaxt="n")
abline(v=c(5.5))
matplot(1:10,t(data1[mat1$cluster==2,]),main="cluster 2", ty="l",
        ylab="alr", col=2, lty=1, xlab="",ylim=c(-3,5.5), xaxt="n")
abline(v=c(5.5))
matplot(1:10,t(data1[mat1$cluster==6,]),main="cluster 6", ty="l",
        ylab="alr", col=6, lty=1, xlab="",ylim=c(-3,5.5), xaxt="n")
abline(v=c(5.5))
matplot(1:10,t(data1[mat1$cluster==11,]),main="cluster 11", ty="l",
        ylab="alr", col=11, lty=1, xlab="",ylim=c(-3,5.5), xaxt="n")
abline(v=c(5.5))
par(mar=c(1,4.1,0.1,0.1))

matplot(1:12,t(data2[mat1$cluster==1,]),main="", ty="l",
        ylab="mprofiles", col=1, lty=1, xlab="",ylim=c(0,1), xaxt="n")
abline(v=c(6.5))
matplot(1:12,t(data2[mat1$cluster==2,]),main="", ty="l",
        ylab="mprofiles", col=2, lty=1, xlab="",ylim=c(0,1), xaxt="n")
abline(v=c(6.5))
matplot(1:12,t(data2[mat1$cluster==6,]),main="", ty="l",
        ylab="mprofiles", col=6, lty=1, xlab="",ylim=c(0,1), xaxt="n")
abline(v=c(6.5))
matplot(1:12,t(data2[mat1$cluster==11,]),main="", ty="l",
